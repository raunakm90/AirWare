...
def train_clf_loso(pipe, clf_params, model_path=MODEL_PATH):
    ...
    write_results_models(train_scores, test_scores, lab_enc.classes_, y_true, y_hat, model_path)
...
def strat_shuffle_split(x, y, split=0.6, random_state=12345):
    cv_obj = StratifiedShuffleSplit(n_splits=1, train_size=split, random_state=random_state)
...
def train_clf_user_calibrated(pipe, clf_params, train_size=0.6, model_path=MODEL_PATH):
    ...
    write_results_models(train_scores, test_scores, lab_enc.classes_, y_true, y_pred, model_path)
