import numpy as np
import argparse
import utils.doppler_models as doppler_model
from utils.doppler_model_helper import *


def run_split_model_1(gest_set, cv_strategy):
    model_fn = doppler_model.split_model_1
    ...
    if cv_strategy == 'loso':
        loso_cv(model_fn, gest_set=gest_set, hyper_param_path=hyper_param_path,
                results_file_path="./leave_one_subject/doppler" + model_result_path)
    elif cv_strategy == 'personalized':
        personalized_cv(model_fn, gest_set=gest_set, hyper_param_path=hyper_param_path,
                        results_file_path="./personalized_cv/doppler" + model_result_path)
...
def run_split_model_2(gest_set, cv_strategy):
    model_fn = doppler_model.split_model_2
...
def run_split_model_3(gest_set, cv_strategy):
    model_fn = doppler_model.split_model_3
...
def run_split_model_4(gest_set, cv_strategy):
    model_fn = doppler_model.split_model_4
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="AirWare grid search and train model using different CV strategies using only Doppler audio signatures")
